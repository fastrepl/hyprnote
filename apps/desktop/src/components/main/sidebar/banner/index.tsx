...
// Store the banner visibility state outside component to persist across re-renders
...
function useShouldShowBanner(isProfileExpanded: boolean) {
  ...
  useEffect(() => {
    // If we haven't started the timer yet or it was cleared
    if (!globalShowBanner && !globalBannerTimer) {
      ...
      // If already shown, update state immediately
      setShowBanner(true);
    }

    return () => {
      // Don't clear the timer on unmount to persist the state
    };
  }, []);
