...
// Import these from the same package structure
// Assuming these exist in the UI package - if not, they need to be moved/created
...
interface UIMessageComponentProps {
  ...
  parseMarkdownBlocks?: (text: string) => ParsedPart[];
  MarkdownCard?: FC<{
    content: string;
    isComplete: boolean;
    sessionTitle?: string;
...
const TextContent: FC<{
  ...
  isHtml?: boolean;
  convertMarkdownToHtml?: (markdown: string) => Promise<string>;
}> = ({ content, isHtml, convertMarkdownToHtml }) => {
  ...
  useEffect(() => {
    const processContent = async () => {
      ...
      if (convertMarkdownToHtml) {
        try {
          let html = await convertMarkdownToHtml(content);
        ...
        // Fallback: display as plain text in a paragraph
        setDisplayHtml(`<p>${content}</p>`);
      }
    };
    ...
    <div className="w-full space-y-4">
      {message.parts?.map((part: any, index: number) => {
        ...
        if (part.type === "text" && part.text) {
          // Use provided parser or default to simple text display
          const parsedParts = parseMarkdownBlocks
            ? parseMarkdownBlocks(part.text)
            : [{ type: "text" as const, content: part.text }];

          return (
            <div key={`${message.id}-text-${index}`} className="space-y-4">
              {parsedParts.map((parsedPart, pIndex) => {
                if (parsedPart.type === "markdown" && MarkdownCard) {
                  return (
                    <MarkdownCard
                      key={`md-${pIndex}`}
