...
- **Connecting...** means this device has not finished setting up encrypted sync. It usually clears within a minute. If it does not, Anarlog shows **Sync needs attention** with the underlying error below the status.
- **Cloud sync delayed** means this device is rebuilding its synced copy and a step keeps failing. The failing step and error appear below the status.

To fix it:

1. Check that the device is online and that a firewall, VPN, or proxy is not blocking `api.anarlog.so`.
...
5. If it still does not clear, open **Settings → Sync**, expand **Sync log**, and email [founders@anarlog.so](mailto:founders@anarlog.so) with the status text, the error shown under it, and the log entries.
