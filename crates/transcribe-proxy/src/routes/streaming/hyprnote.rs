...
fn build_listen_params(params: &QueryParams) -> ListenParams {
    let mut listen_params = params.build_listen_params();
    listen_params.sample_rate = parse_param(params, "sample_rate", 16000);
    listen_params.channels = parse_param(params, "channels", 1);
    listen_params
}
