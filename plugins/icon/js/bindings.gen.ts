...
async getAvailableIcons(isPro: boolean) : Promise<Result<IconVariant[], string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("plugin:icon|get_available_icons", { isPro }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type IconVariant = "beta" | "dark" | "light" | "pro"
