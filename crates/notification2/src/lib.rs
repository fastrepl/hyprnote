...
const NOTIFICATION_HEIGHT_COLLAPSED: Pixels = px(72.);
const NOTIFICATION_HEIGHT_EXPANDED: Pixels = px(500.);
const NOTIFICATION_MARGIN_X: Pixels = px(16.);
const NOTIFICATION_MARGIN_Y: Pixels = px(12.);
...
mod tests {
    ...
    fn test_toast_builder() {
        let toast = StatusToast::new("Test Title")
            ...
            .expanded_content("This is expanded content");
        ...
        assert_eq!(
            toast.expanded_content.as_ref().map(|s| s.as_ref()),
            Some("This is expanded content")
        );
    }
}
