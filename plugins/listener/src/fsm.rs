...
impl Session {
    ...
    async fn setup_resources(&mut self, id: impl Into<String>) -> Result<(), crate::Error> {
        ...
        //             let speaker_chunk = vec![0.0; mic_chunk.len()]; // 더미 데이터 생성
        ...
        // 채널 송신자를 명시적으로 drop하여 수신자가 EOF를 받도록 함
        drop(process_tx);

        // tracing::info!("📍tasks count before from_audio: {}", tasks.len());
        let _listen_stream = listen_client.from_audio(audio_stream).await?;
        // tracing::info!("📍tasks count after from_audio: {}", tasks.len());
