...
impl Workspace {
    ...
    fn render_transcript(
        ...
        let live_ids: std::collections::HashSet<String> = preview
            .transcripts
            .iter()
            .flat_map(|t| t.segments.iter().map(|s| s.id.clone()))
            .collect();
        self.transcript_layouts
            ...
            .children(
                preview
                    ...
                    .map(|(index, transcript)| {
                        ...
                        let offset_ms = crate::transcript::timeline_offset_ms(
    ...
    fn render_transcript_segment(
        ...
        {
            highlights.push(crate::prose_text::Highlight {
                range,
                color: if theme.dark {
        ...
        let text = crate::prose_text::ProseText::with_layout(
            segment.text.clone(),
            runs,
            px(14.0),
                ...
                div()
                    ...
                    .on_mouse_move(
                        cx.listener(move |this, event: &gpui::MouseMoveEvent, _, cx| {
                            ...
                            if this.transcript_hover != next {
                                this.transcript_hover = next;
                                cx.notify();
                            }
                        }),
                    )
                    .on_hover(cx.listener(|this, hovered: &bool, _, cx| {
                        if !*hovered && this.transcript_hover.take().is_some() {
                            cx.notify();
                        }
                    }))
                    ...
                    .child(crate::prose_text::HighlightedProseText::new(
                        text, highlights,
                    )),
            )
            .into_any_element()
