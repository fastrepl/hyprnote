...
export const Route = createFileRoute("/app/daily/$date")({
  beforeLoad: async ({ context: { queryClient, userId }, params: { date } }) => {
    const existingSession = await queryClient.fetchQuery({
      queryKey: ["daily", date],
      queryFn: () => dbCommands.getSession({ daily: { date, user_id: userId } }),
    });

    if (existingSession) {
      throw redirect({ to: "/app/note/$id", params: { id: existingSession.id } });
    }
    ...
    throw redirect({ to: "/app/note/$id", params: { id: newSession.id } });
  },
});
