...
use ws_client::client::{DecodeError, WebSocketClient, WebSocketIO};
...
async fn test_basic_echo() {
    ...
    let (output, _handle, _send_task) = client.from_audio::<TestIO, _>(None, stream).await.unwrap();
...
async fn test_finalize() {
    ...
    let (output, handle, _send_task) = client.from_audio::<TestIO, _>(None, stream).await.unwrap();
