...
pub trait ListenerPluginExt<R: tauri::Runtime> {
    ...
    fn set_default_speaker_device(&self) -> impl Future<Output = Result<(), crate::Error>>;
...
impl<R: tauri::Runtime, T: tauri::Manager<R>> ListenerPluginExt<R> for T {
    ...
    #[tracing::instrument(skip_all)]
    async fn get_current_speaker_device(&self) -> Result<Option<String>, crate::Error> {
        let state: tauri::State<'_, crate::SharedState> = self.state::<crate::SharedState>();
        let s = state.inner().lock().await;
        Ok(s.fsm.get_current_speaker_device())
    }

    /// Set active microphone device by name.
    #[tracing::instrument(skip_all)]
    ...
    async fn set_speaker_device(&self, device_name: impl Into<String>) -> Result<(), crate::Error> {
        ...
        {
            let mut guard = state.inner().lock().await;
            let event = crate::fsm::StateEvent::SpeakerChange(Some(device_name.into()));
            guard.fsm.handle(&event).await;
        }
    ...
    #[tracing::instrument(skip_all)]
    async fn set_default_speaker_device(&self) -> Result<(), crate::Error> {
        let state: tauri::State<'_, crate::SharedState> = self.state::<crate::SharedState>();
        let default = hypr_audio::AudioInput::get_default_speaker_device_name();
        {
            let mut guard = state.inner().lock().await;
            let event = crate::fsm::StateEvent::SpeakerChange(Some(default));
            guard.fsm.handle(&event).await;
        }
        Ok(())
    }
    ...
    async fn check_microphone_access(&self) -> Result<bool, crate::Error> {
        ...
        {
            let mut input = hypr_audio::AudioInput::from_mic(None)?;
