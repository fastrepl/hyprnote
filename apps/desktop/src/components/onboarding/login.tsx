...
export function Login({ onNavigate }: StepProps) {
  ...
  return (
    ...
    >
      <button
        onClick={() => auth?.signIn()}
        className="w-full py-3 rounded-full bg-muted text-foreground text-sm font-medium duration-150 hover:scale-[1.01] active:scale-[0.99]"
      >
      ...
      <div className="relative flex items-center border rounded-full overflow-hidden transition-all duration-200 border-border focus-within:border-ring">
        <input
          type="text"
          className="flex-1 px-4 py-3 text-xs font-mono outline-hidden bg-background"
        ...
        <button
          ...
          className="absolute right-0.5 px-4 py-2 text-sm bg-primary text-primary-foreground rounded-full enabled:hover:scale-[1.02] enabled:active:scale-[0.98] transition-all disabled:opacity-50"
        >
          Submit
        </button>
      </div>
